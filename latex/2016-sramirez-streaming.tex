...
\begin{tabular}{ lcccc }
\toprule
{\bf Method} & {\bf Selection type (measure)} & {\bf Streaming features (conversion)} & {\bf Concept-Evolution}\\
\midrule
...
%
%Although Beringer and H\"ullermeier's method is instructive in that it differentiates its IS strategy based on whether concept drift is reported, their instance deletion strategy might be difficult to implement in problem domains such as spam filtering where all features may be binary. Removing cases temporally may also result in loss of case-base competence, e.g., deleting rare but correct cases.
%
%In~\cite{zlio11}, the authors invented a family of training set formation methods named FISH (uniFied IS algoritHm) based on ensembles of classifiers. The FISH family dynamically selects a set of relevant instances as the training set for the current target instance. The key concept of the FISH family is to linearly combine distances in time and feature space for training set selection. It includes three modifications: FISH1, FISH2 and FISH3. In FISH1, the size of a training set is fixed and set in advance. FISH2, which is considered to be central to the family, varies the size by selecting a training set that achieves the best accuracy based on leave-one-out cross validation. In FISH3, the relative importance of time and space distance is determined through an additional loop of cross validation. Although the FISH family is reported to be capable of cooperating with any base classifier, the validation set is chosen based on nearest neighbors to the current target instance. 
%
...
\scriptsize
<<<<<<< HEAD
\caption{Parameters of methods. Default values for each block of methods are detailed in first rows. Unless specified, these values are common to every method in block.}
=======
...
\label{tab:parameters}
  ...
  \hline
<<<<<<< HEAD
  \textbf{Feature selection} & window size = 1 (default)\\
  NB & --\\
  IG~\cite{kata05} & --\\
  SU~\cite{nguyen12} & --\\
  OFS~\cite{wang14} & $\eta$ = 0.2, $\lambda$ = 0.01\\
  \hline
  \textbf{Instance selection} & k = 3, window size = 100 (default)\\
  kNN & window size = 1\\  
  NEFCS-SRR~\cite{lu16} & l = 10, pmax = 0.5, size limit = 1,000\\
  CBE~\cite{delany05} & --\\
  ICF~\cite{brighton02} & --\\
  FISH~\cite{zlio11} & learner = kNN, distance proportion (time/space) = 0.5, window size = 1\\
  %Learn++.NSE & learner = kNN, slope = 0.5, crossing point = 10, ensemble size = 15, pruning = no\\
  \hline
  \textbf{Discretization} & initial elements = 100, window size = 1 (default)\\
  NB & --\\
  OC~\cite{lehti12} &  --\\
  PiD~\cite{gama06} & $\alpha$ = 0.75, initial bins = 500, instances to update layer \#2 = 10,000, min/max = 0/1\\
  \hline
=======
...
%Finally, Learn++.NSE creates a new classifier and adds it to the ensemble whenever a new batch arrives. 
...
%
%Once the experimental framework and its design is defined, we analyze the experimental results derived from it. This analysis has been divided in three sections according to the different data reductions tasks studied: FS, IS, and discretization.
...
\begin{table*}[!t]
  \caption{Final test accuracy by method (FS). The best outcome for
...
% represents the most appropriate solution for FS processing as best-ranked. According to the Friedman-Holm evaluation, the difference in accuracy between \textit{SU-1,000} and the rest of methods is only statistically significant in case ten features are selected. 
...
% & $+$ & $\pm$ & & \\
%\toprule
%NB & \cellcolor[gray]{0.8} 1 & \cellcolor[gray]{0.8} 3 & 7 & -- \\
%OFS-10 & 0 & 2 & 16.7143 & 0.063201 \\ 
%InfoGain-10 & 0 & \cellcolor[gray]{0.8} 3 & 17.1429 & 0.063201 \\ 
%SU-10 & 0 & \cellcolor[gray]{0.8} 3 & 17.1429 & 0.063201 \\ 
%\hline
%\end{tabular}
%}
%\end{table}
...
% & $+$ & $\pm$ & & \\
%\toprule
%SU-100 & \cellcolor[gray]{0.8} 0 & \cellcolor[gray]{0.8} 3 & 11.6429 & --\\
%OFS-100 & \cellcolor[gray]{0.8} 0 & \cellcolor[gray]{0.8} 3 & 11.9286 & 0.017455 \\ 
%NB & \cellcolor[gray]{0.8} 0 & \cellcolor[gray]{0.8} 3 & 17.2143 & 0.615351 \\
%InfoGain-100 & \cellcolor[gray]{0.8} 0 & \cellcolor[gray]{0.8} 3 & 17.2143 & 0.615351 \\ 
%\hline
%\end{tabular}
%}
%\end{table}
...
\toprule
<<<<<<< HEAD
SU-1000 & \cellcolor[gray]{0.8} 0 & \cellcolor[gray]{0.8} 3 & 10.5714 & -- \\
OFS-1000 & \cellcolor[gray]{0.8} 0 & \cellcolor[gray]{0.8} 3 & 14.1429 & 0.487181 \\ 
NB & \cellcolor[gray]{0.8} 0 & \cellcolor[gray]{0.8} 3 & 16.5714 & 0.487181\\
InfoGain-1000 & \cellcolor[gray]{0.8} 0 & \cellcolor[gray]{0.8} 3 & 16.7143 & 0.487181\\ 
\hline
\end{tabular}
}
\caption{1,000 feature selected}
\end{subtable}
\caption{Wilcoxon test results and average rankings of feature selectors (Friedman Procedure \& Adjusted p-value with Holm's Test) for accuracy}
\label{tab:friedman-fsel}
\end{table}
...
\begin{subfigure}{.5\textwidth}
  ...
  \label{fig:red-fsel}
\end{subfigure}
\caption{Box-plot representation for selection time and reduction (FS)}
\label{fig:fsel}
\end{figure}
...
%\begin{table*}[!t]
%  \caption{Selection time elapsed by method (FS)} \label{tab:time-fsel}
%  \centering
%  \resizebox{.75\textheight}{!}{
%\begin{tabular}{|l|rrrrrrrrrr|}
%\toprule
...
%\begin{table*}[!t]
%  \caption{Reduction rate performed by the different FS schemes} \label{tab:red-fsel}
%  \centering
...
\begin{itemize}
	...
	\item Selection methods makes decisions about the relevance or disturbance of a given instance without knowing how it will be in the future. It is normal that the no-selection option always performs better than others. It only depends on the amount of noise introduced by each problem, and not by other factors like redundancy. %In counterpart, kNN is always updated, and it is only influenced by noise, not by redundancy.
\end{itemize}


\begin{table*}[!t]
  \caption{Total test accuracy by method (IS)} \label{tab:acc-isel}
  \centering
  \resizebox{.75\textheight}{!}{
...
%
%\begin{table*}[!t]
%  \caption{Selection time elapsed by method (IS)} \label{tab:time-isel}
%  \centering
%  \resizebox{.75\textheight}{!}{
...
%\begin{table*}[!t]
%  \caption{\# instances selected by method (IS)} \label{tab:inst-isel}
%  \centering
%  \resizebox{.75\textheight}{!}{
%\begin{tabular}{|l|rrrrrr|}
%\toprule
...
<<<<<<< HEAD
\textit{airlines} & 61.2112 & 55.9120 & \textbf{64.5504} \\ 
\textit{powersupply} & 2.9237 & \textbf{34.5964} & 16.1087 \\ 
\textit{elecNormNew} & 42.6532 & 42.9842 & \textbf{73.3625} \\ 
\textit{spambase} & \textbf{97.8700} & 91.9148 & 82.8081 \\ 
\textit{kddcup\_10} & 66.6213 & 93.5460 & \textbf{97.1908} \\ 
\textit{poker-lsn} & 55.1783 & 32.3913 & \textbf{59.5528} \\ 
\textit{covtypeNorm} & 47.5484 & 4.1266 & \textbf{60.5208} \\ 
\textit{blips} & 47.1230 & 26.5778 & \textbf{60.9060} \\ 
\textit{sudden\_drift} & 66.5752 & 60.8494 & \textbf{83.8144} \\ 
\textit{gradual\_drift} & 51.4210 & 62.1242 & \textbf{84.7000} \\ 
\textit{gradual\_recurring\_drift} & 37.8390 & 25.7000 & \textbf{56.7450} \\ 
\textit{incremental\_fast} & 49.3582 & 43.3580 & \textbf{76.3642} \\ 
\textit{incremental\_slow} & 49.7734 & 49.1084 & \textbf{78.0688} \\ \midrule
\textbf{MEAN} & 52.0074 & 47.9376 & \textbf{68.8225} \\ 
=======
...
\toprule
<<<<<<< HEAD
Na\"ive Bayes & \cellcolor[gray]{0.8} 2 & \cellcolor[gray]{0.8} 2 & 9.3846 & --\\
PiD & 0 & 1 & 23.8462 & 0.001222\\
OC & 0 & 1 & 26.7692 & 0.000203\\
=======
