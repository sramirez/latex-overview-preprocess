...
\centering
\scriptsize
\caption{Summary description for classification datasets}
\label{tab:datasets}
\resizebox{0.75\textwidth}{!}{
  \begin{tabular}{|l||c||c||c||c||c||c|}
  \hline {\bf Data Set} & {\bf \#Ex.} & {\bf \#Atts.} & {\bf \#Num.} & {\bf \#Nom.} &{\bf \#Cl.} & {\bf ?artificial}\\
  \hline
...
\label{tab:parameters}
  ...
  \hline
  \textbf{Feature selection} & window size = 1 (common parameters)\\
  NB & --\\
  IG & --\\
  SI & --\\
  OFS & $\eta$ = 0.2, $\lambda$ = 0.01\\
  \hline
  \textbf{Discretization} & initial elements = 100, window size = 1 (common parameters)\\
  NB & --\\
  OC &  --\\
  PiD & $\alpha$ = 0.75, initial bins = 500, instances to update layer \#2 = 10,000, min/max = 0/1\\
  \hline
  \textbf{Instance selection} & k = 3, window size = 100 (common parameters)\\
  kNN & window size = 1\\  
...
\begin{subfigure}{.5\textwidth}
  ...
  \caption{Reduction rate performed}
  \label{fig:sfig2}
\end{subfigure}
...
\begin{table*}[!t]
  \caption{Total test accuracy by method (Feature Selection)} \label{tab:acc-fsel}
  \centering
  \resizebox{.75\textheight}{!}{
\begin{tabular}{|l|rrrrrrrrrr|}
\toprule
